...
exports.calculateInstallmentPlan = async (req, res) => {
  try {
    ...
    // Ensure whole share purchase
    const parsedQuantity = Math.floor(Number(quantity));
    if (parsedQuantity < 1) {
      return res.status(400).json({
        success: false,
        message: 'Minimum purchase is 1 whole share.'
      });
    }

    // Validate installment months 
    // Minimum 2 months (20%) - Maximum 12 months 
    ...
    const purchaseDetails = await Share.calculatePurchase(parsedQuantity, currency);
    ...
    // Minimum 20% down payment calculation
    const minimumDownPaymentMonths = 2; // 20%
    ...
    const installmentPlan = {
      ...
      currency,
      installmentMonths,
      minimumDownPaymentMonths, // Explicitly show minimum down payment months
      minimumDownPaymentPercentage: (minimumDownPaymentMonths / installmentMonths) * 100, // 20%
      installmentAmount,
...
exports.createInstallmentPlan = async (req, res) => {
  try {
    ...
    // Minimum 20% (2 months) down payment 
    const minimumDownPaymentMonths = 2;
    const minimumDownPaymentPercentage = (minimumDownPaymentMonths / installmentMonths) * 100;
    ...
    for (let i = 0; i < installmentMonths; i++) {
      ...
      installments.push({
        ...
        percentageOfTotal: installmentPercentage,
        sharesReleased, // Number of shares released in this installment
    ...
    const newPlan = new InstallmentPlan({
      ...
      currency,
      installmentMonths,
      minimumDownPaymentMonths, // New field
      minimumDownPaymentPercentage, // New field
      lateFeePercentage: lateFee,
    ...
    res.status(201).json({
      ...
      plan: {
        ...
        currency,
        installmentMonths,
        minimumDownPaymentMonths, // Return minimum down payment info
        minimumDownPaymentPercentage, // Return minimum down payment percentage
...
exports.getUserInstallmentPlans = async (req, res) => {
  try {
    ...
    const formattedPlans = plans.map(plan => ({
      ...
      minimumDownPaymentMonths: plan.minimumDownPaymentMonths,
...
exports.submitManualInstallmentPayment = async (req, res) => {
  try {
    ...
    // Validate minimum payment (20%)
    const minimumInstallmentCount = plan.minimumDownPaymentMonths || 2;
    const isPastMinimumPayment = 
      plan.installments.filter(i => i.status === 'paid').length >= minimumInstallmentCount;
    
    if (!isPastMinimumPayment) {
      return res.status(400).json({
        success: false,
        message: `You must complete at least the first ${minimumInstallmentCount} installments before skipping or using alternative payment methods.`
      });
    }
...
exports.cancelInstallmentPlan = async (req, res) => {
  try {
    ...
    // Additional checks for cancellation
    if (plan.status === 'cancelled') {
      return res.status(400).json({
        success: false,
    ...
    // Validate minimum down payment requirement before cancellation
    const minimumDownPaymentMonths = plan.minimumDownPaymentMonths || 2;
    const paidInstallments = plan.installments.filter(i => i.status === 'paid').length;
    
    if (paidInstallments < minimumDownPaymentMonths) {
      return res.status(400).json({
        success: false,
        message: `You cannot cancel the plan before completing the first ${minimumDownPaymentMonths} installments.`
      });
    }
