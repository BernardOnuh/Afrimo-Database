 ...
 * @route GET /api/withdrawal/admin/settings
 * @access Admin
 */
...
/**
 * Update global withdrawal settings
 * @route PUT /api/withdrawal/admin/settings
 * @access Admin
 */
exports.updateWithdrawalSettings = async (req, res) => {
  try {
    ...
    if (globalWithdrawalEnabled !== undefined) updateData.globalWithdrawalEnabled = globalWithdrawalEnabled;
    if (minimumWithdrawalAmount !== undefined) updateData.minimumWithdrawalAmount = minimumWithdrawalAmount;
    if (maxDailyWithdrawals !== undefined) updateData.maxDailyWithdrawals = maxDailyWithdrawals;
    if (withdrawalFeePercentage !== undefined) updateData.withdrawalFeePercentage = withdrawalFeePercentage;
...
/**
 * Toggle global withdrawal status
 * @route POST /api/withdrawal/admin/toggle-global
 * @access Admin
 */
exports.toggleGlobalWithdrawals = async (req, res) => {
  try {
    ...
    if (enabled === undefined) {
      return res.status(400).json({
        success: false,
        message: 'Please specify whether to enable or disable withdrawals'
      });
    }
    ...
    const settings = await WithdrawalSettings.findOneAndUpdate(
      {},
      {
        globalWithdrawalEnabled: enabled,
        lastUpdated: new Date(),
        updatedBy: req.user.id,
        ...(reason && { reason })
      },
    ...
    res.status(200).json({
      success: true,
      message: enabled ? 'Withdrawals enabled globally' : 'Withdrawals disabled globally',
      data: {
        globalWithdrawalEnabled: settings.globalWithdrawalEnabled,
        reason: reason || null,
        updatedBy: req.user.id,
        updatedAt: settings.lastUpdated
      }
    });
  } catch (error) {
    console.error('Error toggling global withdrawals:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to toggle global withdrawal status',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
...
/**
 * Toggle withdrawal status for a specific user
 * @route POST /api/withdrawal/admin/user/:userId/toggle
 * @access Admin
 */
exports.toggleUserWithdrawals = async (req, res) => {
  try {
    ...
    if (disabled === undefined) {
      return res.status(400).json({
        success: false,
        message: 'Please specify whether to disable or enable withdrawals for this user'
      });
    }
    ...
    if (disabled) {
      // Disable withdrawals for user
      const restriction = await UserWithdrawalRestriction.findOneAndUpdate(
        { user: userId },
        {
      ...
      res.status(200).json({
        success: true,
        message: 'Withdrawals disabled for user',
        data: restriction
      });
    } else {
      ...
      if (!restriction) {
        return res.status(404).json({
          success: false,
          message: 'No withdrawal restriction found for this user'
        });
      }

      res.status(200).json({
        success: true,
        message: 'Withdrawals enabled for user',
        data: restriction
      });
    }
  } catch (error) {
    console.error('Error toggling user withdrawals:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to toggle user withdrawal status',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
...
/**
 * Get withdrawal status for a specific user
 * @route GET /api/withdrawal/admin/user/:userId/status
 * @access Admin
 */
exports.getUserWithdrawalStatus = async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    // Get global settings
    const globalSettings = await WithdrawalSettings.findOne();
    const globalEnabled = globalSettings ? globalSettings.globalWithdrawalEnabled : true;

    // Get user-specific restriction
    const restriction = await UserWithdrawalRestriction.findOne({ user: userId });
    const userDisabled = restriction ? restriction.withdrawalDisabled : false;

    res.status(200).json({
      success: true,
      data: {
        userId: userId,
        withdrawalDisabled: userDisabled,
        canWithdraw: globalEnabled && !userDisabled,
        restriction: restriction,
        globalStatus: globalEnabled
      }
    });
  } catch (error) {
    console.error('Error getting user withdrawal status:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to get user withdrawal status',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
...
/**
 * Get list of users with withdrawal restrictions
 * @route GET /api/withdrawal/admin/restricted-users
 * @access Admin
 */
exports.getRestrictedUsers = async (req, res) => {
  try {
    ...
    const restrictions = await UserWithdrawalRestriction.find({ withdrawalDisabled: true })
    ...
    const count = await UserWithdrawalRestriction.countDocuments({ withdrawalDisabled: true });
...
/**
 * Get complete withdrawal history for all users (Admin only)
 * @route GET /api/withdrawal/admin/complete-history
 * @access Admin
 */
exports.getCompleteWithdrawalHistory = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 20,
    ...
    // Add facet for pagination and summary
    pipeline.push({
      $facet: {
        data: [
          { $sort: sort },
    ...
    res.status(200).json({
      ...
      summary: {
        ...
        averageAmount: Math.round((summary.averageAmount || 0) * 100) / 100,
        statusBreakdown
      }
    });
  } catch (error) {
    console.error('Error fetching complete withdrawal history:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch withdrawal history',
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
...
/**
 * Enhanced middleware to check withdrawal permissions
 * Checks both global settings and user-specific restrictions
 */
exports.checkWithdrawalPermissions = async (req, res, next) => {
  try {
    const userId = req.user.id;

    // Check global settings
    const globalSettings = await WithdrawalSettings.findOne();
    if (globalSettings && !globalSettings.globalWithdrawalEnabled) {
      return res.status(403).json({
        success: false,
        message: 'Withdrawals are currently disabled globally. Please contact support.',
        code: 'WITHDRAWALS_GLOBALLY_DISABLED'
      });
    }

    // Check user-specific restrictions
    const userRestriction = await UserWithdrawalRestriction.findOne({ 
      user: userId, 
      withdrawalDisabled: true 
    });
    ...
    if (userRestriction) {
      return res.status(403).json({
        success: false,
        message: `Withdrawals are disabled for your account. Reason: ${userRestriction.reason}`,
        code: 'USER_WITHDRAWALS_DISABLED',
        restriction: {
          reason: userRestriction.reason,
          disabledAt: userRestriction.disabledAt
        }
      });
    }

    // Check daily withdrawal limits if configured
    if (globalSettings && globalSettings.maxDailyWithdrawals) {
      ...
      if (todayWithdrawals >= globalSettings.maxDailyWithdrawals) {
        return res.status(400).json({
          success: false,
          message: `You have reached the daily withdrawal limit of ${globalSettings.maxDailyWithdrawals} withdrawals.`,
    ...
    console.error('Error checking withdrawal permissions:', error);
    return res.status(500).json({
      success: false,
...
/**
 * Get user's current earnings balance with withdrawal restrictions info
 * @route GET /api/withdrawal/balance
 * @access Private
 */
exports.getEarningsBalanceWithRestrictions = async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    const minimumWithdrawalAmount = globalSettings?.minimumWithdrawalAmount || MINIMUM_WITHDRAWAL_AMOUNT;
    const globalWithdrawalEnabled = globalSettings?.globalWithdrawalEnabled !== false;
    const userWithdrawalEnabled = !userRestriction?.withdrawalDisabled;
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          withdrawalRestrictions: {
            globalEnabled: globalWithdrawalEnabled,
            userEnabled: userWithdrawalEnabled,
            reason: userRestriction?.reason || null,
            maxDailyWithdrawals: globalSettings?.maxDailyWithdrawals || null
          }
        }
      });
    }
    ...
    let dailyWithdrawalsUsed = 0;
    if (globalSettings?.maxDailyWithdrawals) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);

      dailyWithdrawalsUsed = await Withdrawal.countDocuments({
        user: userId,
        createdAt: { $gte: today, $lt: tomorrow },
        status: { $in: ['pending', 'processing', 'paid'] }
      });
    ...
    const canWithdraw = globalWithdrawalEnabled && 
                       userWithdrawalEnabled && 
                       availableBalance >= minimumWithdrawalAmount && 
                       pendingWithdrawals === 0 && 
                       processingWithdrawals === 0 &&
                       (!globalSettings?.maxDailyWithdrawals || dailyWithdrawalsUsed < globalSettings.maxDailyWithdrawals);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        withdrawalRestrictions: {
          ...
          reason: userRestriction?.reason || null,
          maxDailyWithdrawals: globalSettings?.maxDailyWithdrawals || null,
          dailyWithdrawalsUsed,
          dailyWithdrawalsRemaining: globalSettings?.maxDailyWithdrawals ? 
            Math.max(0, globalSettings.maxDailyWithdrawals - dailyWithdrawalsUsed) : null
        }
      }
    });
  } catch (error) {
